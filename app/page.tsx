...
export default async function Home() {
  ...
  const city = data && data.location && data.location.name ? data.location.name : null;
  const region = data && data.location && data.location.region ? data.location.region : null;
  const country = data && data.location && data.location.country ? data.location.country : null;
    ...
    <main className="flex min-h-screen flex-col items-center p-6 lg:p-24 text-neutral-100">
      <div className="max-w-8xl">
        ...
        <div className="flex flex-col mt-4 justify-center px-6 py-3.5 bg-white/10 backdrop-blur rounded-2xl border border-white min-h-10">
          ...
          <p className="text-xs opacity-50 mt-2">Last updated on: {data.current.last_updated}</p>
          <div className="flex w-full justify-between gap-2 text-sm mt-2">
            <p>Feels like: {data.current.feelslike_c}&deg;C</p>
            <p>Wind: {data.current.wind_dir} {data.current.wind_kph} km/h</p>
